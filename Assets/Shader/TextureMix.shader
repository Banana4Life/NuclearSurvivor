...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float4 node_6660 = normalize(float4(i.vertexColor.r,i.vertexColor.g,i.vertexColor.b,i.vertexColor.a));
                float3 _Texture0Normal_var = UnpackNormal(tex2D(_Texture0Normal,TRANSFORM_TEX(i.uv0, _Texture0Normal)));
                float3 _Texture1Normal_var = UnpackNormal(tex2D(_Texture1Normal,TRANSFORM_TEX(i.uv0, _Texture1Normal)));
                float3 _Texture2Normal_var = UnpackNormal(tex2D(_Texture2Normal,TRANSFORM_TEX(i.uv0, _Texture2Normal)));
                float3 _Texture3Normal_var = UnpackNormal(tex2D(_Texture3Normal,TRANSFORM_TEX(i.uv0, _Texture3Normal)));
                float3 normalLocal = (lerp( lerp( lerp( lerp( _Texture0Normal_var.rgb, _Texture0Normal_var.rgb, node_6660.r ), _Texture1Normal_var.rgb, node_6660.g ), _Texture2Normal_var.rgb, node_6660.b ), _Texture3Normal_var.rgb, node_6660.a ));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                ...
                float3 specularColor = _Metallic;
                float specularMonochrome;
                float4 _Texture0_var = tex2D(_Texture0,TRANSFORM_TEX(i.uv0, _Texture0));
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.vertexColor = v.vertexColor;
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityMetaVertexPosition(v.vertex, v.texcoord1.xy, v.texcoord2.xy, unity_LightmapST, unity_DynamicLightmapST );                return o;
            }
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _Texture2_var = tex2D(_Texture2,TRANSFORM_TEX(i.uv0, _Texture2));
                float4 _Texture3_var = tex2D(_Texture3,TRANSFORM_TEX(i.uv0, _Texture3));
                float3 diffColor = (lerp( lerp( lerp( lerp( _Texture0_var.rgb, _Texture0_var.rgb, node_6660.r ), _Texture1_var.rgb, node_6660.g ), _Texture2_var.rgb, node_6660.b ), _Texture3_var.rgb, node_6660.a ));
                float specularMonochrome;
                float3 specColor;
